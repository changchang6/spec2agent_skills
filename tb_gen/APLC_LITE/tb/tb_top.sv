...
module tb_top;
	crg_if u_crg_if();
     ...
     // bind APLC_LITE crg_if u_crg_if (
     //);
	//spi_if u_spi_if(.clk(u_crg_if.clks[0]),.rst_n(u_crg_if.rsts[0]));
  //fifo_if u_fifo_if(.clk(u_crg_if.clks[0]),.rst_n(u_crg_if.rsts[0]));
  //csr_if u_csr_if(.clk(u_crg_if.clks[0]),.rst_n(u_crg_if.rsts[0]));
   ...
   assign u_dut.clk = u_crg_if.clks[0];
   assign u_dut.rst_n = u_crg_if.rsts[0];
	bind APLC_LITE if_harness harness(.*);
	initial begin
		run_test();
	end
   ...
   u_dut.harness.set_fifo_vif("uvm_test_top.m_env.csr_agt[0].m_monitor");
   u_dut.harness.set_csr_vif("uvm_test_top.m_env.csr_agt[0].m_monitor");
   u_dut.harness.set_fifo_vif("uvm_test_top.m_env.fifo_agt[0].m_monitor");
   u_dut.harness.set_csr_vif("uvm_test_top.m_env.fifo_agt[0].m_monitor");
	end
   ...
   //   uvm_config_db#(virtual fifo_if)::set(uvm_root::get(), "*", "vif", u_fifo_if);
